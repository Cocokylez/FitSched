...
type StreakWelcomeCardProps = {
  ...
  /** Streak freezes left this calendar month (auto-applied when a day is missed). */
...
export function StreakWelcomeCard({
  ...
  useEffect(() => {
    ...
    // Show this moment at most ONCE PER DAY (per state), not on every app open.
    // localStorage persists across app restarts; sessionStorage did not, which
    // is why the card re-popped every time the app was reopened.
  ...
  const title = hasActiveStreak
    ? `${displayStreak} day streak is alive`
    : "Your streak went quiet"
  const showFreezeChip = typeof freezesRemaining === "number"
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <FireBurst broken={missedStreak} streak={displayStreak} introPhase={introPhase} />
            ...
            >
              <div style={{ fontSize: 11, fontWeight: 900, letterSpacing: "0.17em", color: "rgba(255, 255, 255, 0.48)", textTransform: "uppercase", marginBottom: 7 }}>
                {missedStreak ? "Recovery moment" : "Welcome back"}
              </div>
              ...
              {/* Streak freezes are applied automatically when you miss a day. */}
              {showFreezeChip && (
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 14, padding: "6px 12px", borderRadius: 999, background: "rgba(147, 225, 250, 0.12)", border: "1px solid rgba(147, 225, 250, 0.22)", color: "#bfeaff", fontSize: 12, fontWeight: 800 }}>
                  <Snowflake size={13} strokeWidth={2.4} />
                  {freezesRemaining} streak {freezesRemaining === 1 ? "freeze" : "freezes"} left this month
                </div>
              )}
            </motion.div>
