  ...
  return (
    <div style={{ minHeight: "100vh", background: ["linear-gradient(180deg, rgba(18,101,254,0.12) 0%, transparent 38vh)", "radial-gradient(circle at 18% -8rem, rgba(18,101,254,0.18), transparent 28rem)", "radial-gradient(circle at 88% 65%, rgba(18,101,254,0.07), transparent 22rem)", "var(--bg)"].join(", "), display: "flex", flexDirection: "column", position: "relative", padding: "0 16px 24px" }}>
      ...
      >
        ...
        <div style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "6px" }}>{t.fpRecovery}</div>
      </motion.div>
          ...
          {googleAccount ? (
            <>
              <motion.div variants={fadeIn}>
                ...
                <div className="display-text" style={{ fontSize: "22px", fontWeight: 800, color: "var(--text)", marginBottom: 8 }}>{t.fpGoogleTitle}</div>
                <div style={{ fontSize: "13px", color: "var(--text-muted)", lineHeight: 1.6, marginBottom: 28 }}>
                  <strong style={{ color: "var(--text)" }}>{email}</strong> {t.fpGoogleBody}
                </div>
              </motion.div>
              <motion.div variants={fadeIn}>
            ...
            <>
              <motion.div variants={fadeIn}>
                ...
                <div className="display-text" style={{ fontSize: "22px", fontWeight: 800, color: "var(--text)", marginBottom: 8 }}>{t.fpCheckTitle}</div>
                <div style={{ fontSize: "13px", color: "var(--text-muted)", lineHeight: 1.6, marginBottom: 28 }}>
                  {t.fpCheckPre} <strong style={{ color: "var(--text)" }}>{email}</strong> {t.fpCheckPost}
                </div>
                <div style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: 20 }}>
                  {t.fpDidntGet}{" "}
                  <span style={{ color: "var(--text)", fontWeight: 600, cursor: "pointer" }} onClick={() => setSent(false)}>{t.fpTryAgain}</span>.
                </div>
              </motion.div>
              <motion.div variants={fadeIn}>
                <button
            ...
            <>
              <motion.div variants={fadeIn}>
                <div className="display-text" style={{ fontSize: "24px", fontWeight: 800, color: "var(--text)", marginBottom: 4 }}>{t.fpForgotTitle}</div>
                <div style={{ fontSize: "13px", color: "var(--text-muted)", marginBottom: 24 }}>{t.fpEnterEmail}</div>
              </motion.div>
              ...
              <motion.div variants={fadeIn}>
                <p style={{ textAlign: "center", fontSize: "12px", color: "var(--text-muted)", margin: 0 }}>
                  {t.fpRememberIt}{" "}
                  <Link href="/login" style={{ color: "var(--text)", fontWeight: 700, textDecoration: "none" }}>{t.signIn}</Link>
                </p>
              </motion.div>
            </>
          )}
