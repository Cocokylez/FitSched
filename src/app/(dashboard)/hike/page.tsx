...
import {
  Activity, ArrowUpRight, Footprints, Mountain,
  MapPin, Plus, Timer, Trash2, TrendingUp, X,
} from "lucide-react"
...
function formatPace(distanceKm: number, durationMin: number): string {
  ...
  const h = Math.floor(minPerKm / 60)
  const m = Math.floor(minPerKm % 60)
  const s = Math.round((minPerKm - Math.floor(minPerKm)) * 60)
  if (h > 0) return `${h}:${String(m).padStart(2, "0")}/km`
...
export default function HikePage() {
  ...
  const canSubmit = !!distanceKm && Number(distanceKm) > 0 &&
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible">
          ...
          <motion.div variants={fadeUp} style={{ marginBottom: 22 }}>
            ...
            <p style={{ color: "var(--text-muted)", fontSize: 14, lineHeight: 1.55, margin: 0, maxWidth: 400 }}>
              Track your hikes, distance, and elevation — separate from gym sessions.
            </p>
          </motion.div>
          ...
          <motion.div variants={fadeUp}>
            {loading ? (
              ...
              <div style={{
                ...
                <Mountain
                  size={40} strokeWidth={1.4} color={ACCENT}
                  style={{ marginBottom: 14, opacity: 0.65, display: "block", margin: "0 auto 14px" }}
                />
